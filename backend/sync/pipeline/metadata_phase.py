...
from funcy import chunks, silent

from common.config import SyncConfig
from sync.data.models import MetadataPassResult, MetadataPhaseOutcome, PageMetadata
...
async def execute_metadata_phase(
    *,
    ...
    # Important: do NOT persist page state in the metadata phase.
    # Doing so would incorrectly mark pages as synced before Weaviate writes
    # complete, preventing recovery on subsequent runs.
...
async def run_metadata_pass(
    ...
    *,
    ...
    # Decide which UIDs require fetching state from Weaviate:
    #  - UIDs without any preloaded state (we need last_synced + existence)
    #  - UIDs with preloaded DB rows but unknown page_objects where DB indicates
    #    they are up-to-date (we verify existence to safely skip)
    state_fetch_uids: List[str] = []
    for uid in remaining_after_since:
        ...
        if aggregated is None:
            continue
        pre_state = (preloaded_state or {}).get(uid)
        if not pre_state:
            state_fetch_uids.append(uid)
            continue
        if pre_state.get("page_objects"):
            # Already confirmed existence; no fetch needed
            continue
        stored_edit = TO_INT(pre_state.get("last_synced_edit_time")) if pre_state.get("last_synced_edit_time") is not None else None
        if stored_edit is not None and stored_edit >= aggregated:
            # Candidate for skip, but we must confirm Weaviate actually has the page
            state_fetch_uids.append(uid)
